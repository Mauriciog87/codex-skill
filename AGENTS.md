...
`playwright` keeps repository files unchanged and uses the private `sol_luna_playwright` MCP, pinned to `@playwright/mcp@0.0.80`, in an isolated temporary environment. The installer preserves user MCP entries. The launcher checks the package pin in effective configuration separately from the advertised runtime version `1.63.0-alpha-2026-08-31`, then verifies required tools and successful thread-and-turn-scoped calls; text mentions do not count. It never calls `browser_run_code_unsafe`. Full interaction is limited to localhost and named dev/test targets. External state changes require explicit destination-specific authorization, and purchases, deletion, publishing, messaging, account/security changes, or production mutation are prohibited.
