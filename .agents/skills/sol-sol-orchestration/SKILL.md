...
description: Coordinate substantive software work with GPT-5.6 Sol at xhigh reasoning as orchestrator and verified GPT-5.6 Sol executors at high reasoning. Invoke explicitly at the start of every new substantive root Codex task and when planning, delegating, coordinating, or validating independent work.
---

# Sol Sol Orchestration

Run this workflow once before planning the first substantive request in a root task. Do not run it inside a session whose developer instructions contain `CODEX_ORCHESTRATION_ROLE=executor`.
...
5. Send each executor briefing through the skill's Node launcher on stdin. Include the objective, exclusions, owned files or subsystem, expected result, and required checks.
...
7. Accept an executor result only when the launcher exits successfully, reports `gpt-5.6-sol` with `high` reasoning, and sets `routing_verified` to `true`.
...
```text
briefing | node .agents/skills/sol-sol-orchestration/scripts/invoke-sol-executor.mjs --cwd <repository> --sandbox read-only --timeout-seconds 900
```

Run a write-enabled executor only for an assigned file scope:

```text
briefing | node .agents/skills/sol-sol-orchestration/scripts/invoke-sol-executor.mjs --cwd <repository> --sandbox workspace-write --timeout-seconds 900
```

The launcher reads the briefing from stdin, fixes the executor to `gpt-5.6-sol` with `high`, disables the `multi_agent` feature, and validates the session rollout. Codex 0.144.0 requires `agents.max_depth` to be at least `1`, so the launcher uses that valid minimum while disabling the feature itself. Exit code `0` means completed with verified routing, `1` means the task or Codex execution failed, and `2` means the invocation, timeout, configuration, output contract, or routing verification failed.
